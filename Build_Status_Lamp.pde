...
void loop()
{
  ...
  //  GlowLED(GREEN_LED);
  //for(int count =0; count < 5100; count++)
  //  GlowUpdateLED(GREEN_LED);
...
void GlowLED(int LEDId)
{
  int fadeCounter;
  
  for(fadeCounter = 0; fadeCounter < 255; fadeCounter++)
  {
    analogWrite(LEDId, fadeCounter);
    delay(10);
  }
  
  for(fadeCounter = 255; fadeCounter > 0; fadeCounter--)
  {
    analogWrite(LEDId, fadeCounter);
    delay(10);
  }
